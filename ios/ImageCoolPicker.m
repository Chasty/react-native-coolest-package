#import "ImageCoolPicker.h"
#import "ImageCoolPickerUtils.h"
...
@implementation ImageCoolPicker
...
- (void)launchImagePicker:(NSDictionary *)options callback:(RCTResponseSenderBlock)callback
{
    ...
    if (target == camera && [ImageCoolPickerUtils isSimulator]) {
    ...
    if (@available(iOS 14, *)) {
        if (target == library) {
            PHPickerConfiguration *configuration = [ImageCoolPickerUtils makeConfigurationFromOptions:options target:target];
    ...
    [ImageCoolPickerUtils setupPickerFromOptions:picker options:self.options target:target];
    picker.delegate = self;
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    dispatch_block_t dismissCompletionBlock = ^{
        if ([info[UIImagePickerControllerMediaType] isEqualToString:(NSString *) kUTTypeImage]) {
            [self onImageObtained:[ImageCoolPicker getUIImageFromInfo:info]
                             data:[NSData dataWithContentsOfURL:[ImageCoolPicker getNSURLFromInfo:info]]];
        }
        else {
...
- (void)onImageObtained:(UIImage*)image data:(NSData*)data
{
    ...
    NSString *fileType = [ImageCoolPickerUtils getFileType:data];

    if (![fileType isEqualToString:@"gif"]) {
        image = [ImageCoolPickerUtils resizeImage:image
